...
describe("test transfer transcations factory", function () {
    ...
    const transaction = new NextTransferTransactionsFactory(config, new TokenComputer());
    ...
    it("should throw error, no token transfer provided", async () => {
        ...
        assert.throw(
            () => {
                transaction.createTransactionForESDTTokenTransfer({
                    sender: alice,
                    receiver: bob,
    ...
    it("should create 'TransactionNext' for native token transfer without data", async () => {
        const next = transaction.createTransactionForNativeTokenTransfer({
            sender: alice,
            receiver: bob,
            nativeAmount: "1000000000000000000",
        });

        assert.equal(next.sender, alice.bech32());
        assert.equal(next.receiver, bob.bech32());
        assert.equal(next.value.valueOf(), "1000000000000000000");
        assert.equal(next.gasLimit.valueOf(), "50000");
        assert.deepEqual(next.data, new Uint8Array());
    });
    ...
    it("should create 'TransactionNext' for native token transfer with data", async () => {
        ...
        assert.equal(next.receiver, bob.bech32());
        assert.equal(next.value.valueOf(), "1000000000000000000");
        assert.equal(next.gasLimit.valueOf(), "63500");
        assert.deepEqual(next.data, Buffer.from("test data"));
    });
    ...
    it("should create 'TransactionNext' for esdt transfer", async () => {
        ...
        const next = transaction.createTransactionForESDTTokenTransfer({
            sender: alice,
            receiver: bob,
        ...
        assert.equal(next.sender, alice.bech32());
        assert.equal(next.receiver, bob.bech32());
        assert.equal(next.value.valueOf(), "0");
        assert.equal(next.gasLimit.valueOf(), "410000");
        assert.deepEqual(next.data.toString(), "ESDTTransfer@464f4f2d313233343536@0f4240");
    });
    ...
    it("should create 'TransactionNext' for nft transfer", async () => {
        ...
        assert.equal(next.sender, alice.bech32());
        assert.equal(next.receiver, alice.bech32());
        assert.equal(next.value.valueOf(), "0");
        assert.equal(next.gasLimit.valueOf(), "1210500");
        assert.deepEqual(
            next.data.toString(),
    ...
    it("should create 'TransactionNext' for multiple nft transfers", async () => {
        ...
        assert.equal(next.receiver, alice.bech32());
        assert.equal(next.value.valueOf(), "0");
        assert.equal(next.gasLimit.valueOf(), "1466000");
        assert.deepEqual(
            next.data.toString(),
